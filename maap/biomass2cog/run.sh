...
# Derive per-polarization power (intensity) COGs from the *_amp.tif COGs that
# biomass2cog just wrote, alongside them in the output directory. Best-effort:
# a failure here (e.g. no amp COGs on an amp-only=false run) must not fail the job.
conda run --live-stream -p /opt/conda/envs/rift_biomass2cog \
  python "${basedir}/../../scripts/make_pwr_cogs.py" "${OUTDIR}" --output-dir "${OUTDIR}" \
  || echo "WARNING: make_pwr_cogs.py failed; continuing without power COGs" >&2
