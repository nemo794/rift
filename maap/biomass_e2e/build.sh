#!/usr/bin/env bash
################################################################################
# MAAP build script for the rift biomass-e2e OGC algorithm.
#
...
# envs in the image:
#
#   rift_biomass2cog : heavy isce3/biomass geocoding env (rift biomass2cog +
#                      biomass-reader + sardem + ESA-access deps + boto3)
#   crevasse         : the nisar-crevasse gate + U-Net inference env (torch)
#
# and bakes the large BIOMASS U-Net checkpoint from my-public-bucket into the
# cloned nisar-crevasse repo at the exact nested path its predictor loads by
# default (bio_unet_predict.py's DEFAULT_CHECKPOINT, under models/BIOMASS/...).
# my-public-bucket is NOT anonymously public (403 over HTTPS), so the model is
# pulled with credentials (MAAP workspace creds first, then the default AWS
# chain) via boto3.
################################################################################
set -euo pipefail
...
# --------------------------------------------------------------------------------------
# Env A: heavy BIOMASS geocoding (rift biomass2cog + isce3 + biomass-reader)
# --------------------------------------------------------------------------------------
# Built first so the credentialed model fetch below can run inside it — it ships boto3 +
# maap-py, and the base maap_base env does not.
...
# --------------------------------------------------------------------------------------
# Fetch the large BIOMASS U-Net checkpoint from my-public-bucket (FAIL EARLY).
# --------------------------------------------------------------------------------------
# The credentialed S3 fetch is the most failure-prone step, so do it right after the
# geocoding env — before the heavy crevasse solve and the clone — and stage it to a temp
# file. It is copied into the clone at its final nested path once that exists (see "Bake
# the model" below).
#
...
# (has boto3 + maap-py).
MODEL_S3_URI="${MODEL_S3_URI:-s3://maap-ops-workspace/shared/niemoell/crevasse_unet_models/biomass/unet_best.safetensors}"
MODEL_STAGED="$(mktemp /tmp/bio_unet_best.XXXXXX.safetensors)"

echo "Fetching BIOMASS U-Net checkpoint: ${MODEL_S3_URI}"
...
# --------------------------------------------------------------------------------------
# Clone the merged crevasse repo (both sensors; we only drive the BIOMASS side).
...
# --------------------------------------------------------------------------------------
# Must match bio_unet_predict.py's DEFAULT_CHECKPOINT:
#   models/biomass/unet/bio_unet_4ch_frangi_aux_nw0p02/unet_best.safetensors
# (BIOMASS-specific directory — NOT the nisar/ one). The RF gate (bio_gate_t512.joblib,
# ~13 MB) is git-tracked in the crevasse repo and ships in the clone, so only this one
# large U-Net safetensors needs fetching.
MODEL_DEST="${CREVASSE_DIR}/models/biomass/unet/bio_unet_4ch_frangi_aux_nw0p02/unet_best.safetensors"
...
import rift
print("Geocoding env OK — isce3", isce3.__version__, "| rasterio", rasterio.__version__)
...
import os
import torch
from crevasse.biomass.bio_unet_predict import DEFAULT_CHECKPOINT
p = str(DEFAULT_CHECKPOINT) + ".safetensors"
assert os.path.exists(p), f"baked BIOMASS U-Net checkpoint missing at {p}"
print("Crevasse env OK — torch", torch.__version__)
print("BIOMASS U-Net checkpoint present:", p)
PY
conda run -p "${CREVASSE_ENV_PREFIX}" python "${CHECK_ML_PY}"
rm -f "${CHECK_ML_PY}"

echo "✓ rift biomass-e2e image ready (envs: rift_biomass2cog + crevasse; model baked)"
